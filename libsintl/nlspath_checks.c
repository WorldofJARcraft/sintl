...
int
nls_safe_open(const char *path, libsintl_statbuf *statbuf, int *trust, int safe)
{
	int	fd;
	int	trust_path;
	int	systemdir = 0;
	...
	if (libsintl_fstat64(fd, statbuf) == -1) {
		(void) close(fd);
		return (-1);
	}
...
clean_env(void)
{
	...
	if (!*p)				/* None found, we're safe */
		nlspath_safe = 1;
	else if (libsintl_issetugid()) {			/* Found and set-uid, clean */
		int off = 1;
